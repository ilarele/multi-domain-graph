    ...
    def apply_expert_batch(self, batch_rgb_frames):
        ...
        with torch.no_grad():
            ...
            for tuple in self.combine_list:
                first_elem = tuple[0]
                for elem in tuple[1:]:
                    class_labels[class_labels == elem] = first_elem

            mask = torch.zeros(class_labels.shape).cuda()
            for i in range(len(self.to_keep_list)):
                m = class_labels == self.to_keep_list[i]
                class_labels[m] = i + 1
                mask = mask + m
            mask = mask == 0
            class_labels[mask] = 0
